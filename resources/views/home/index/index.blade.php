...
<div class="fengtui">
    @foreach($ftLists as $v)
        <dl>
            <dt>
                <a href="{!! bookurl($v['cate_id'],$v['id']) !!}" title="{{ $v['title'] }}"><img
                            src="{!! bookimg($v['thumb']) !!}" alt="{{ $v['title'] }}"></a>
            </dt>
            <dd>
                <h3>
                    <a href="{!! bookurl($v['cate_id'],$v['id']) !!}"
        ...
        <div class="tuijian">
            ...
            <ul @if($k == 1 || $k%4 == 1) class="l" @endif>
                ...
                @foreach($v['data'] as $vv)
                    <li><a href="{!! bookurl($vv['cate_id'],$vv['id']) !!}"
                           title="{{ $vv['title'] }}">{{ mb_substr($vv['title'],0,7) }}</a>/{{ mb_substr($vv['author'],0,5) }}
                    </li>
                @endforeach
            </ul>
            @if($k == 4 || $k%4 == 0)
    ...
    <div class="lastupdate">
        ...
        <ul>
            ...
            @foreach($newLists as $v)
                <li>
                    <span class="lx">[<a href="{!! bookurl($v['cate_id']) !!}">{{ $v['catname'] }}</a>]</span>
                    <span class="sm">
                    <a href="{!! bookurl($v['cate_id'],$v['id']) !!}"
        ...
        <ul>
            @foreach($newInserts as $v)
                <li>
                    ...
                    <span class="sm">
                        <a href="{!! bookurl($v['cate_id'],$v['id']) !!}"
                           title="{{ $v['title'] }}">{{ mb_substr($v['title'],0,6,'utf-8') }}</a>
                    </span>
